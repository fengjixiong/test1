...
{
	int id, // ID
	enum_struct_type type, // ID对应的类型
	void* pointer // ID对应的指针
}st_id_info_t;
st_id_info_t table_id_info[MAX_POINTS*10];

//查找里程对应的资源	
int find_licheng_info(st_licheng_info** info_table, int licheng);	//返回info_table的行，-1表示失败
//查找ID对应的指针	
int find_id_info(st_id_info** info_table, int id);	//返回info_table的行，-1表示失败
